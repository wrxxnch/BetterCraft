...
function mcl_end.detach_chorus_plant(start_pos, digger)
	...
	for t=1, #tree_start_posses do
		...
		-- chorus plant nodes.
		local touched_nodes_hashes = { core.hash_node_position(start_pos) }
		...
		while #check_posses > 0 do
			...
			if not touched_nodes_hashes[core.hash_node_position(pos)] then
				local node = core.get_node(pos)
				touched_nodes_hashes[core.hash_node_position(pos)] = true
				if node.name == "mcl_end:end_stone" then
					...
					break_tree = false
					break
				elseif core.get_item_group(node.name, "chorus_plant") == 1 then
					table.insert(chorus_nodes, pos)
					for i=1, #neighbors do
						local newpos = vector.add(pos, neighbors[i])
						if not touched_nodes_hashes[core.hash_node_position(newpos)] then
		...
		if break_tree then
			...
			for c=1, #chorus_nodes do
				no_detach[ core.hash_node_position(chorus_nodes[c]) ] = true
				if digger then
					core.node_dig(chorus_nodes[c], { name = "mcl_end:chorus_plant" }, digger)
				else
					core.remove_node(chorus_nodes[c])
				end
			end
		end
	end
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		elseif node_below.name == "air" then
			...
			for a=1, #around do
				...
				local node_side = core.get_node(pos_side)
		...
		if plant_ok then
			...
			local it, suc = core.item_place_node(itemstack, placer, pointed_thing)
			if suc then
				core.sound_play(mcl_sounds.node_sound_wood_defaults().place, {pos = pos}, true)
			end
			return it
		else
			return itemstack
...
core.register_node("mcl_end:chorus_plant", {
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		local plant_ok = false
		if node_check.name == "mcl_end:end_stone" or core.get_item_group(node_check.name, "chorus_plant") > 0 then
			plant_ok = true
		end
		if plant_ok then
			-- Placement OK! Proceed normally
			local it, suc = core.item_place_node(itemstack, placer, pointed_thing)
			if suc then
				core.sound_play(mcl_sounds.node_sound_wood_defaults().place, {pos = pos_place}, true)
			end
			return it
		else
			return itemstack
...
function mcl_end.grow_chorus_plant(pos, node, pr)
	...
	if node.name ~= "mcl_end:chorus_flower" then
		core.set_node(pos, { name = "mcl_end:chorus_flower" })
	end
	while true do
		local new_flowers_list = {}
		for f=1, #flowers do
			local new_flowers = mcl_end.grow_chorus_plant_step(flowers[f], core.get_node(flowers[f]), pr)
...
function mcl_end.grow_chorus_plant_step(pos, node, pr)
	...
	for a=1, #around do
		if core.get_node(vector.add(above, around[a])).name ~= "air" then
			air_around = false
			break
		end
	end
		...
		for y=1, 4 do
			...
			if node.name == "mcl_end:chorus_plant" then
				...
				if not branching then
					for a=1, #around do
						local node_side = core.get_node(vector.add(checkpos, around[a]))
						if node_side.name == "mcl_end:chorus_plant" then
							branching = true
						end
					end
		...
		if grow_chance then
			...
			if r <= grow_chance then
				...
				for _ = 1, branches do
					...
					if core.get_node(below_branch).name == "air" then
			...
			for _, f in pairs(new_flowers) do
				if age >= MAX_FLOWER_AGE then
					local nn = core.get_node(f).name
					if nn ~= "mcl_end:chorus_flower" and nn ~= "mcl_end:chorus_flower_dead" then
						core.set_node(f, {name="mcl_end:chorus_flower_dead"})
						grown = true
					end
				else
					local nn = core.get_node(f).name
					if nn ~= "mcl_end:chorus_flower" and nn ~= "mcl_end:chorus_flower_dead" then
						core.set_node(f, {name="mcl_end:chorus_flower", param2 = age})
			...
			if #new_flowers >= 1 then
				core.set_node(pos, {name="mcl_end:chorus_plant"})
				grown = true
			end
		end
	end
	if not grown then
		...
		core.set_node(pos, {name = "mcl_end:chorus_flower_dead"})
	end
...
core.register_craftitem("mcl_end:chorus_fruit_popped", {
