...
-- Suporte a tradução compatível com várias versões do engine
if minetest.get_translator_auto then
    mesecon.S = minetest.get_translator_auto("mesecons")
elseif minetest.get_translator then
    mesecon.S = minetest.get_translator("mesecons")
else
    -- fallback: sem sistema de tradução, retorna string original
    mesecon.S = function(s) return s end
end
