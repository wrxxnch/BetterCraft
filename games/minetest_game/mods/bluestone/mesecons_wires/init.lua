...
-- Remove spawn natural da bluestone e da stone_with_mese

-- Impede spawn natural de todos os nodes mesecons:wire_XXXX_on/off
    ...
    for x = minp.x, maxp.x do
        for y = minp.y, maxp.y do
            for z = minp.z, maxp.z do
                ...
                if node.name:match("^mesecons:wire_.*_on$") or node.name:match("^mesecons:wire_.*_off$") then
        ...
        for x = minp.x, maxp.x do
            for y = minp.y, maxp.y do
                for z = minp.z, maxp.z do
                    local pos = {x=x, y=y, z=z}
                    local node = minetest.get_node(pos)
                    if node.name == "default:stone_with_mese" then
                        minetest.set_node(pos, {name="air"})
                    end
                end
            end
