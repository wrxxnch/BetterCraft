...
local function register_switchkey()
    ...
    controls.register_on_press(function(player, control_name)
        ...
        if vel and (vel.x ~= 0 or vel.z ~= 0) then
            -- jogador está se movendo → correr
            local physics = player:get_physics_override()
            player:set_physics_override({
                speed = (physics.speed or 1) * 1.8, -- aumenta 80%
            })
            return
        end

        -- jogador parado → checar se mainhand é item do i_have_hands
        local main_item = player:get_wielded_item()
        local main_name = (main_item and main_item:get_name()) or ""
        if main_name:match("^i_have_hands:") then
            minetest.chat_send_player(player:get_player_name(),
                "Não é possível trocar para offhand enquanto segura um item do i_have_hands.")
            return
        end
    ...
    -- Quando soltar o botão, volta ao normal
...
minetest.item_place = function(mainhand_stack, player, pointed_thing, ...)
    ...
    if not is_switched
        ...
        -- se o item na mão for do i_have_hands, não usar offhand
        local main_name = mainhand_stack and mainhand_stack:get_name() or ""
        if main_name:match("^i_have_hands:") then
            return item_place(mainhand_stack, player, pointed_thing, ...)
        end
...
controls.register_on_press(function(player, control_name)
    ...
    -- tool já implementa handler -> pular
    if mainhand and mainhand.on_secondary_use then return end
    -- não é ferramenta -> pular
    if stackname ~= "" and minetest.registered_tools[stackname] == nil then return end
    -- olhando para o chão -> pular
    if get_pointed_thing(player, mainhand) ~= nil then return end

    -- se o mainhand for item do i_have_hands, não acionar o offhand
    local main_name = (mainhand and mainhand:get_name()) or ""
    if main_name:match("^i_have_hands:") then
        return
    end
...
minetest.register_globalstep(function(dtime)
    for _, player in pairs(minetest.get_connected_players()) do
        ...
        local offhand_player = offhand[player]
        ...
        if offhand_item ~= "" and item then
            ...
            if offhand_hud.wear_bar then
                if offhand_player.last_wear ~= offhand_get_wear(player) then
                    update_wear_bar(player, itemstack)
                    offhand_player.last_wear = offhand_get_wear(player)
                end
            ...
            if offhand_hud.item_count then
                if offhand_player.last_count ~= offhand_get_count(player) then
                    player:hud_change(offhand_hud.item_count, "text", offhand_get_count(player))
                    offhand_player.last_count = offhand_get_count(player)
                end
...
minetest.register_allow_player_inventory_action(function(player, action, inventory, inventory_info)
    if action == "move" and inventory_info.to_list == "offhand" then
        ...
        if name:match("^i_have_hands:") then
            -- proíbe mover itens do i_have_hands para offhand
            return 0
        end
